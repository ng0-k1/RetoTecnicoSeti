...
def crear_nodo_asesor_vehiculo(llm):
    """Retorna el nodo LangGraph especializado en análisis de crédito vehicular.
    ...
    def nodo(estado: dict) -> dict:
        ...
        logger.info("[vehiculo] ruta=%s prompt_chars=%d", ruta, len(prompt))
        ...
        try:
            ...
            logger.info("[vehiculo] respuesta_chars=%d", len(contenido))
            return {"analisis": contenido}
        except Exception as exc:
            logger.exception("[vehiculo] error en el agente: %s", exc)
            raise
