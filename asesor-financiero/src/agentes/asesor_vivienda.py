...
def crear_nodo_asesor_vivienda(llm):
    """Retorna el nodo LangGraph especializado en analisis de credito hipotecario."""
    def nodo(estado: dict) -> dict:
        ...
        tasas_raw = buscar_web.invoke("tasas credito hipotecario vivienda Colombia 2026")
        logger.info("[vivienda] busqueda_mercado preview=%r", tasas_raw[:100])
        tasas_mercado = escapar_llaves(tasas_raw[:500])

        instituciones_raw = buscar_web.invoke(
            "tasas credito hipotecario FNA Banco Agrario Bancolombia Davivienda BBVA Colombia 2026"
        )
        logger.info("[vivienda] busqueda_instituciones preview=%r", instituciones_raw[:100])
        ...
        logger.info("[vivienda] ruta=%s prompt_chars=%d", ruta, len(prompt_usuario))
